...
metadata %>% 
  filter(!is.na(Sex)) %>%
  group_by(Temperature, Treatment, DPE, Sex) %>%
  count() %>%
  flextable()
...
```{r Diet-Beta-Temp, fig.height=6, fig.width=6}


dist <- distList.conT0TF

# Full dbRDA models
mod <- full_dbrda_model(vars = c("Temperature"),  # Enter variables as a list "c(var.A, var.B, var.C)"
                          distance = dist,
...
data.alpha %>%
  filter(Alpha.Metric == "Simpson" | Alpha.Metric == "Phylogenetic") %>%
    ggplot(aes(x = as.factor(DPE), 
               y = Alpha.Score
               )) +
    geom_boxplot(aes(fill = Temperature,
...
lapply(methods.beta, function(beta){
  ...
  dt.ord.full.beta[[beta]]$sample.coords %>%
    mutate(Temp.DPE = paste0(Temperature,"_",DPE)) %>%
    ggplot(aes(x = CAP1, y = CAP2)) +
      stat_ellipse(aes(linetype = Temperature)) +
      ...
      #     arrow = arrow(length = unit(0.05, "npc"))
      # ) +
      scale_fill_manual(values = col.TempDPE, name = "Temp & DPE") +
      scale_shape_manual(values = c(21:25)) +
      scale_color_manual(values = "black" ) +
      labs(x = dt.ord.full.beta[[beta]]$axes.labs[1],
           y = dt.ord.full.beta[[beta]]$axes.labs[2],
           caption = paste0(beta, "; F=",tmp.stat,"; P", ifelse(tmp.pval <= 0.001, "<", "="),tmp.pval )) +
      theme(legend.position = "bottom") +
      guides(fill=guide_legend(override.aes = list(shape = 21, fill = col.TempDPE )))
