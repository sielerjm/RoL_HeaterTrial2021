...
ps.raw <- readRDS(paste0(data.path, "/Output/dada2_1.24.0_2022-09-12/phyloseq.rds"))
...
sample_data(ps.raw)$Sample <- factor(sample_data(ps.raw)$Sample, levels=sort(rownames(sample.data.frame(ps.raw))))

sample_data(ps.raw) <- sample_data(ps.raw)[order(sample_data(ps.raw)$Sample),]
